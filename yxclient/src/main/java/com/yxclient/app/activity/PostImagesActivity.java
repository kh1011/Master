    ...
    static class MyRunnable implements Runnable {
        ...
        public void run() {
            ...
            for (int i = 0; i < images.size(); i++) {
                if (images.get(i).contains(DemoApplication.getInstance().getString(R.string.glide_plus_icon_string))) {//说明是添加图片按钮
                    continue;
                }
                //newBitmap= ImageUtils.getBitmap(images.get(i));
                //newBitmap = ImageUploadUtils.compressImage(newBitmap);
                //newBitmap= zuo.biao.library.util.ImageUtils.getSmallBitmap(images.get(i));
                //压缩
                newBitmap = ImageUtils.compressScaleByWH(images.get(i),
                        DensityUtils.dp2px(DemoApplication.getInstance().getApplicationContext(), 1000),
                        DensityUtils.dp2px(DemoApplication.getInstance().getApplicationContext(), 1000));
                //文件地址
