...
import static com.yxclient.app.R.id.tabbar;
import static com.yxclient.app.config.YXConfig.token;
...
public class YXMainActivity extends BaseActivity implements BadgeDismissListener, OnTabSelectListener {
    ...
    public void initView() {
        setTitle("主页");
        registId = JPushInterface.getRegistrationID(this);
        System.out.println("极光注册ID：" + registId);
        // 上传极光注册ID
        ...
        if (!StringUtil.isNullOrEmpty(DemoApplication.getInstance().getMyToken())) {
            uploadID(DemoApplication.getInstance().getMyToken(), registId, "");
        }
    }
