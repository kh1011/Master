package com.yxdriver.app.recorduitl;
...
public class Util {
	...
	public static List<AMapLocation> parseLatLngAMapLocation(List<LatLng> list) {
		List<AMapLocation> traceList = new ArrayList<AMapLocation>();
		if (list == null) {
			return traceList;
		}
		for (int i = 0; i < list.size(); i++) {
			LatLng loc = list.get(i);
