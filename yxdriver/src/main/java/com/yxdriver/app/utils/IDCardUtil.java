...
/**
 * 检查身份证是否合法
 * Created by zhangyun on 2017/9/21 0021
...
public class IDCardUtil {
    ...
    public static boolean validateIDCard(String idNum) {
        ...
        if (idNumLength != 15 && idNumLength != 18) {
            Log.d(TAG,"身份证号码应该为15位或18位");
            return false;
        }
        //对身份证的字符做判断
        if (!isAllNum(idNum)) {
            Log.d(TAG,(idNum.length() == 18 ? "18位号码除最后一位外,都应为数字" : "15位号码都应为数字"));
            return false;
        }
        if (idNum.contains("x")) {
            Log.d(TAG,("身份证x必须为大写"));
